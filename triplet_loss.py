...
class TripletLoss(object):
    ...
    def __init__(self, margin=None):
        ...
        self.weight_param = nn.Parameter(torch.ones(1, dtype=torch.float).cuda())

    def __call__(self, global_feat, labels, param, normalize_feature=False):
        if normalize_feature:
            global_feat = normalize(global_feat, axis=-1)
        dist_mat = euclidean_dist(global_feat, global_feat)
        dist_ap, dist_an, dist_an_mean, ind_pos, ind_neg = hard_example_mining(
            dist_mat, labels, True)
        neg_weight = self.weight(ind_neg, param, labels)
